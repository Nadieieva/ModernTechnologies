...
def get_port():
    ...
    for i in link_port:
        val1 = soup.find_all('font', class_='spy14')[1].contents[1].contents[0].split('<\\/font>"+')[1].replace('(',
                                                                                                                '').replace(
            ')', '').split('+')
        port = ''
        for j in val1:
...
def get_connection_type():
    ...
    link_type = soup.find_all("a", {
        "href": ["/en/https-ssl-proxy/", "/en/http-proxy-list/", "/en/socks-proxy-list", "/en/socks-proxy-list/"]})
    link_type.pop(0)
    detail_link = []
